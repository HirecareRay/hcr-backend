...
from app.interview import context, llm, stt
...
# 비언어(시선·표정) 분석은 Phase 4(MediaPipe)에서 채운다 — 지금은 자리표시.
_NONVERBAL_PLACEHOLDER = '(비언어 분석은 Phase 4에서 제공됩니다)'
...
async def build_summary(history: tuple[Turn, ...]) -> SummaryEvent:
    """면접 기록으로 최종 통합 리포트를 만든다(LLM 실패 시 안전 기본 요약)."""
...
def _summary_event(data: dict) -> SummaryEvent:
    """LLM 요약 dict 를 SummaryEvent 로 안전 변환한다(누락 필드는 기본값)."""
    return SummaryEvent(
        overall_score=_coerce_score(data.get('overall_score')),
        language_feedback=str(
            data.get('language_feedback') or '평가를 생성하지 못했습니다.'
        ),
        nonverbal_feedback=_NONVERBAL_PLACEHOLDER,
        improvements=[
...
def _coerce_score(value: object) -> float:
    """점수를 float 으로 강제하되 파싱 불가면 0.0 — 잘못된 LLM 출력 방어."""
    try:
