...
질문 생성에 두 소스를 합친다 — (1) 회사 분석(실데이터, 없으면 mock)과
(2) 로그인 지원자의 이력서·포트폴리오. 어느 쪽이 비어도 면접이 끊기지 않도록
조회 실패는 안전 기본값(mock / 빈 문자열)으로 우회한다(데모·테스트 보호).
...
# 회사 분석 데이터가 없을 때 쓰는 mock. 실데이터(company.service)가 주입되면 대체된다.
...
async def get_company_context(
    ...
    하나라도 없거나 조회·조립에 실패하면 회사 무관 mock 으로 우회한다(데모·테스트 보호).
    """
    ...
    try:
        ...
        return context or MOCK_COMPANY_CONTEXT
    except Exception as error:  # noqa: BLE001 - 회사 조회 실패가 면접을 막지 않게
        logger.error('회사 컨텍스트 조회 실패, mock 사용: %s', error)
        return MOCK_COMPANY_CONTEXT
...
def _format_user_context(document: dict) -> str:
    ...
    이력서·포트폴리오·자기소개서의 핵심만 추린다(전문은 토큰 낭비). 포트폴리오는
    저장 시 'projects' 필드 안 'portfolio' 키에 리스트로 담기는 구조를 따른다.
    """
    sections = [
