...
# report_messages(결과 리포트)가 유일한 채점 소스다 — 라이브 요약(계약 ②)도 이 리포트
# 점수를 그대로 파생해 두 화면 종합점수가 항상 일치한다. evaluation_messages(스트림
# 피드백)도 같은 잣대로 톤을 맞춘다(점수는 내지 않음).
SCORING_RUBRIC = (
...
def main_questions_messages(
    ...
    system = (
        ...
        '1개 포함합니다.\n'
        '- 한 질문에는 한 가지만 묻습니다.\n'
        '- 지원자 한 명과 마주 앉은 1:1 면접입니다. 반드시 그 한 사람에게 존댓말로 '
        '묻고, "여러분"·"여러분들"처럼 여러 명을 부르는 표현은 절대 쓰지 않습니다.\n'
...
def follow_up_messages(
    ...
    system = (
        ...
        '지원자 한 명에게 존댓말로 묻고, "여러분" 같은 복수 청자 표현은 쓰지 않습니다.\n\n'
...
def report_messages(transcript: str, job_title: str) -> list[dict[str, str]]:
    ...
    결과 페이지(계약 ④)에 필요한 풍부한 산출물을 1회 호출로 모두 만든다 — 종합 점수·
    답변 피드백·강약점·보완점·턴별 평가·추천 질문. 라이브 요약(계약 ②)도 이 결과에서
    파생하므로, 이 리포트가 면접 채점의 유일한 소스다.
