...
def follow_up_messages(
    ...
    유지한다. 답변이 부실하면(이름·소속만, 너무 짧음, 실질 무응답) 억지 꼬리질문을
    만들지 않고 정확히 ``SKIP`` 한 단어만 출력하도록 한다. 호출부
    (service.generate_follow_up)가 이 SKIP 를 감지해 꼬리질문을 건너뛴다.

    ⚠️ 깐깐 규칙: 답변에 파고들 실질 내용(구체적 경험·근거·수치·사례·판단)이
    있을 때만 꼬리질문을 만들고, 애매하면 무조건 SKIP 한다. 이름·숫자 같은 사소한
    단어 하나를 붙들고 되묻는 헛질문(예: "박초롱입니다" → 이름 관련 질문)을 원천 차단한다.
    """
    system = (
        f'당신은 면접의 {persona.role_label} 입니다. 담당 주제는 [{persona.focus}] '
        f'이고, 말투는 다음과 같습니다: {persona.tone}\n'
        '지원자의 직전 답변에서, 그 답변에 실제로 담긴 구체적 내용(경험·기술·판단·'
        '근거·수치·사례)을 더 깊이 파고드는 꼬리질문을 하나만 생성하세요. 실제 '
        '면접관이 되묻듯 자연스러운 구어체 한 문장으로, 기계적 표현 없이 질문만 '
        '출력합니다.\n\n'
        '아래 중 하나라도 해당하면 억지로 질문을 만들지 말고 정확히 SKIP 한 단어만 '
        '출력하세요(애매하면 무조건 SKIP):\n'
        '- 답변이 이름·소속·인사말 위주다(예: "안녕하세요 박초롱입니다", '
        '"OO대학교에서 왔습니다").\n'
        '- 답변이 한두 마디로 너무 짧거나, 질문에 실질적으로 답하지 않았다.\n'
        '- 답변에 파고들 만한 구체적 내용(경험·근거·수치·사례)이 없다.\n'
        '절대로 답변 속 이름·숫자·사소한 단어 하나를 붙들고 되묻지 마세요 — '
        '예를 들어 "박초롱입니다"라는 답에 이름의 뜻·유래·별명 같은 질문을 만드는 것은 '
        '금지입니다. 그런 경우 반드시 SKIP 하세요.'
    )
