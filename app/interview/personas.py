"""면접 패널 페르소나 — 3인 면접관의 담당 영역·말투·목소리 힌트. 데이터로만 정의한다.

role_label 만 프론트로 노출(질문 배지). focus·tone 은 프롬프트 내부 전용,
voice 는 프론트 TTS(SpeechSynthesis) 폴백이 담당별 목소리로 매핑하는 데 쓴다.
백엔드 ElevenLabs TTS 의 voice_id·발화 설정은 여기가 아니라 설정(core/config.py 의
INTERVIEW_TTS_VOICES)에서 읽는다 — 코드 수정 없이 .env 로 튜닝하기 위함이다.
...
class Persona:
    ...
    id: str          # 안정 식별자(프론트 배지·목소리 매핑, TTS 설정 키)
...
# 페르소나 id 가 곧 TTS 설정(INTERVIEW_TTS_VOICES)의 키다 — 담당별 ElevenLabs voice_id·
# 발화 속도는 core/config.py 에서 읽으므로 여기서는 역할·말투만 정의한다.
CULTURE = Persona(
    id='culture_fit',
    role_label='인사담당자',
    voice='soft_high',
