"""적합성 분석 LLM 프롬프트 — 5단계 독립 Prompt.

각 Prompt는 단일 책임(Single Responsibility)만 가진다.
입력은 원본 MongoDB/MariaDB 데이터 JSON을 그대로 사용한다.
필드 정의·설명은 schemas.py Field(description=...)이 담당한다.
"""

# ─── Stage 1a: Candidate Profile ─────────────────────────────────────
# 입력: hcr.user_documents 문서 JSON
#
# 주요 필드:
#   resume.school[]                   → 학력
#   resume.career[]                   → 경력
#   resume.tools_skills[]             → 기술·도구
#   resume.certifications[]           → 자격증
#   work_experience.work_experience[] → 경력기술서
#   cover_letter.items[]              → 자기소개서
...
- source는 MongoDB 문서 내 실제 JSON 경로입니다
...
# 입력: companies(MongoDB) + company_analyses + company_crawler(MariaDB) 조합 JSON
#
# 주요 필드:
#   company_name, industry     → companies
#   business_description       → company_crawler
#   main_products_services     → company_crawler
#   ceo_message                → company_crawler
#   swot_strengths/weaknesses  → company_analyses
#   jobplanet_review_summary   → company_analyses
#   key_points                 → company_analyses
