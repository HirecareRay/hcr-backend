"""적합성 분석 스키마 — Profile 중심 설계.

흐름:
  Stage 1a/1b/1c (병렬): CandidateProfile / JobProfile / CompanyProfile 생성 → MongoDB 저장
...
Field description은 with_structured_output 시 LLM이 JSON Schema로 수신한다.
"""
...
# ─── 최종 저장/반환 스키마 (DB ID 주입 후) ───────────────────────────

class EvidenceRef(BaseModel):
    doc_id: str = Field(description="candidate_profiles MongoDB _id")
...
class JobMatch(BaseModel):
    job_posting_id: str = Field(description="job_postings._id")
    match_target_type: str
    match_target_text: str
    match_target_evidence: str | None = None
    matched: bool
    candidate_profile_id: str = Field(description="candidate_profiles._id")
    candidate_evidence: EvidenceRef
    reasoning: str | None = None


class CompanyMatch(BaseModel):
    company_profile_id: str = Field(description="company_profiles._id")
    dimension: str
    criterion_text: str
